...
def register_font(font_path):
   """ Windows에서 TTF 폰트를 임시 등록하는 함수 """
   FR_PRIVATE = 0x10  # 개별 프로세스에서만 사용
   FR_NOT_ENUM = 0x20  # 시스템 폰트 목록에 추가하지 않음

   if os.path.exists(font_path):
      ctypes.windll.gdi32.AddFontResourceExW(font_path, FR_PRIVATE | FR_NOT_ENUM, 0)
   return

def register_tkinter_font(root, font_name, size=14):
    """ tkinter에서 폰트를 등록하는 함수 """
    root.tk.call("font", "create", "CustomFont", "-family", font_name, "-size", size)
    
font1_path = resource_path("./fonts/SpoqaHanSansNeo-Bold.ttf")
font2_path = resource_path("./fonts/SpoqaHanSansNeo-Regular.ttf")

register_font(font1_path)
register_font(font2_path)

window.tk.call("font", "create", "Spoqa Han Sans Neo Bold", "-family", "Spoqa Han Sans Bold", "-size", 14)
window.tk.call("font", "create", "Spoqa Han Sans Neo Regular", "-family", "Spoqa Han Sans Neo Regular", "-size", 11)
...
#################################################
def donothing():
   messagebox.showinfo(VERSION,"[editor] \t 김선제 \n\n\t ancestor@korea.kr \n\n\t cowpower@kakao.com \n\n [fonts] \t Spoqa Han Sans fonts \n\n\t https://spoqa.github.io/spoqa-han-sans/")
...
def fn_fileDel():
    try:
        ...
        messagebox.showwarning("주의","삭제할 파일이 없습니다.")
...
def fn_tab1_Run():
   ...
   if filecnt==0:
      messagebox.showinfo(VERSION,"사용법\n\n" +
                                  "단계 1. 파일 추가하기 \n" +
                                  "단계 2. 파일 정렬하기 \n" +
                                  "단계 3. 파일 합치기 \n" +
                                  "단계 4. 결과 파일 확인하기")
   ...
   try:
      ...
      messagebox.showinfo("파일 합치기","합치기 완료")
   ...
   try:
      ...
      if folder !="":
         if filename != "":
            ...
            messagebox.showinfo("이미지로 변환하기","이미지 변환 완료")
   ...
   try:
      ...
      if folder !="":
         if filename != "":
            ...
            messagebox.showinfo("텍스트 파일로 변환","텍스트 파일 변환 완료")
...
def fn_tab4_setPDFFile():
   thread = threading.Thread(target = fn_tab4_setPDFFile_thread)
   thread.daemon = True
   thread.start()
   return

#-------------------------------
def fn_tab4_setPDFFile_thread():
   ...
   try:
      ...
      temp = (100-vb_pb.get())/len(tab4_images)
...
def fn_tab4_Run():
   ...
   try:
      ...
      messagebox.showinfo("페이지 회전하기","저장 완료")
...
def fn_tab5_setPDFFile():
   thread = threading.Thread(target = fn_tab5_setPDFFile_thread)
   thread.daemon = True
   thread.start()
   return

#-------------------------------
def fn_tab5_setPDFFile_thread():
...
def fn_tab5_Run():
   ...
   try:
      ...
      pdfWriter.add_metadata(
         {
            ...
            "/TRASH": "R342322-R255323-R123323-R315522"
         }
      )
      ...
      messagebox.showinfo("페이지 순서변경","페이지 순서변경 완료")
...
def fn_tab6_setWaterMarkFile():
   thread = threading.Thread(target = fn_tab6_setWaterMarkFile_thread)
   thread.daemon = True
   thread.start()
   return

#-------------------------------
def fn_tab6_setWaterMarkFile_thread():
...
def fn_tab6_setPDFFile():
   thread = threading.Thread(target = fn_tab6_setPDFFile_thread)
   thread.daemon = True
   thread.start()
   return

#-------------------------------
def fn_tab6_setPDFFile_thread():
...
def fn_tab6_Save():
   ...
   if not(pdfFile!="" and watermarkFile!=""):
      messagebox.showinfo(VERSION,"사용법\n\n" +
                                  "단계 1. 워터마크 PDF 파일 열기(첫 페이지가 워터마크가 됨)\n" +
                                  "단계 2. PDF 파일 열기\n" +
                                  "단계 3. 저장\n" +
                                  "단계 4. 결과 확인")
      return
   ...
   try:
      ...
      messagebox.showinfo("워터마크","워터마크 설정 완료")
...
style.configure('TNotebook.Tab', foreground="white", background="gray", width=window.winfo_screenwidth(), font=("Spoqa Han Sans Neo Bold","14","bold"), anchor="c")
style.map("TNotebook.Tab", foreground=[("selected", "gray")], 
                           background=[("selected", "white")],
                           font=[("selected",("Spoqa Han Sans Neo Bold","14", "bold"))])
...
tabs.add(tab1, text="파일 합치기")
tabs.add(tab2, text="이미지로 변환")
tabs.add(tab3, text="텍스트로 변환")
tabs.add(tab4, text="페이지 회전")
tabs.add(tab5, text="페이지 순서 변경")
tabs.add(tab6, text="워터마크")
tabs.add(tab7, text="시스템 알림")
...
##    TAB1
#################################################
bt_fileadd = tk.Button(tab1, text='파일 추가', relief="groove", overrelief="solid", command=fn_fileAdd, width=10)
bt_fileadd.grid(row=0, column=0, pady=2, sticky=W+E+N+S)


bt_filedel = tk.Button(tab1, text='파일 삭제', relief="groove", overrelief="solid", command=fn_fileDel, width=10)
bt_filedel.grid(row=0, column=1, pady=2,  sticky=W+E+N+S)

bt_filedelAll = tk.Button(tab1, text='모든 파일 삭제', relief="groove", overrelief="solid", command=fn_filedelAll, width=10)
...
style.configure("mystyle.Treeview.Heading", font=('맑은 고딕', 12,'normal'), relief='ridge') # Modify the font of the headings
...
tab1_tree.heading("Name", text="파일명", anchor=CENTER)
tab1_tree.heading("Size", text="파일크기", anchor = CENTER)
tab1_tree.heading("Update", text="수정일자", anchor = CENTER)


bt_filemerge = tk.Button(tab1, text='저장', relief="groove", overrelief="solid", command=fn_tab1_Run)
...
bt_fileOpen = tk.Button(tab1, text='파일 열기', relief="groove", overrelief="solid", command=fn_fileOpen, width=10)
bt_fileOpen.grid(row=4, column=0, padx=1, pady=2, sticky=W+E+N+S)

bt_folderOpen = tk.Button(tab1, text='폴더 열기', relief="groove", overrelief="solid", command=fn_folderOpen, width=10)
bt_folderOpen.grid(row=4, column=1, padx=1, pady=2, sticky=W+E+N+S)

bt_clear = tk.Button(tab1, text='화면 초기화', relief="groove", overrelief="solid", command=fn_clear, width=10)
...
##    TAB2
#################################################
bt_tab2_setPDFFile = tk.Button(tab2, text='PDF 파일 열기', relief="groove", overrelief="solid", command=fn_tab2_setPDFFile)
...
bt_tab2_setFolder = tk.Button(tab2, text='저장 폴더 설정', relief="groove", overrelief="solid", command=fn_tab2_setFolder)
...
bt_tab2_Run = tk.Button(tab2, text='이미지 파일로 변환하기', relief="groove", overrelief="solid", command=fn_tab2_Run)
...
bt_tab2_fileOpen = tk.Button(tab2, text='파일 열기', relief="groove", overrelief="solid", command=fn_tab2_fileOpen, width=30)
bt_tab2_fileOpen.grid(row=4, column=0, padx=1, pady=2, sticky=W+E+N+S)

bt_tab2_folderOpen = tk.Button(tab2, text='폴더 열기', relief="groove", overrelief="solid", command=fn_tab2_folderOpen, width=30)
bt_tab2_folderOpen.grid(row=4, column=1, padx=1, pady=2, sticky=W+E+N+S)

bt_tab2_clear = tk.Button(tab2, text='화면 초기화', relief="groove", overrelief="solid", command=fn_tab2_clear)
...
##    TAB3
#################################################
bt_tab3_setPDFFile = tk.Button(tab3, text='PDF 파일 열기', relief="groove", overrelief="solid", command=fn_tab3_setPDFFile)
...
bt_tab3_setFolder = tk.Button(tab3, text='저장 폴더 설정', relief="groove", overrelief="solid", command=fn_tab3_setFolder)
...
bt_tab3_Run = tk.Button(tab3, text='텍스트 파일로 변환하기', relief="groove", overrelief="solid", command=fn_tab3_Run)
...
bt_tab3_fileOpen = tk.Button(tab3, text='파일 열기', relief="groove", overrelief="solid", command=fn_tab3_fileOpen, width=30)
bt_tab3_fileOpen.grid(row=4, column=0, padx=1, pady=2, sticky=W+E+N+S)

bt_tab3_folderOpen = tk.Button(tab3, text='폴더 열기', relief="groove", overrelief="solid", command=fn_tab3_folderOpen, width=30)
bt_tab3_folderOpen.grid(row=4, column=1, padx=1, pady=2, sticky=W+E+N+S)

bt_tab3_clear = tk.Button(tab3, text='화면 초기화', relief="groove", overrelief="solid", command=fn_tab3_clear, width=54)
...
##    TAB4
#################################################
bt_tab4_setPDFFile = tk.Button(tab4, text='PDF 파일 열기', relief="groove", overrelief="solid", command=fn_tab4_setPDFFile)
...
list_tab4 = Listbox(tab4, width=30, height=21, activestyle="none", exportselection=False, justify="center") ##, font=('consolas',11,'normal'))
...
bt_tab4_Run = tk.Button(tab4, text='저장', relief="groove", overrelief="solid", command=fn_tab4_Run)
...
bt_tab4_fileOpen = tk.Button(tab4, text='파일 열기', relief="groove", overrelief="solid", command=fn_tab4_fileOpen, width=30)
bt_tab4_fileOpen.grid(row=4, column=0, padx=1, pady=2, sticky=W+E+N+S)

bt_tab4_folderOpen = tk.Button(tab4, text='폴더 열기', relief="groove", overrelief="solid", command=fn_tab4_folderOpen, width=30)
bt_tab4_folderOpen.grid(row=4, column=1, padx=1, pady=2, sticky=W+E+N+S)

bt_tab4_clear = tk.Button(tab4, text='화면 초기화', relief="groove", overrelief="solid", command=fn_tab4_clear)
...
##    TAB5
#################################################
bt_tab5_setPDFFile = tk.Button(tab5, text='PDF 파일 열기', relief="groove", overrelief="solid", command=fn_tab5_setPDFFile, width=30)
...
bt_tab5_Run = tk.Button(tab5, text='저장', relief="groove", overrelief="solid", command=fn_tab5_Run)
...
bt_tab5_fileOpen = tk.Button(tab5, text='파일 열기', relief="groove", overrelief="solid", command=fn_tab5_fileOpen, width=30)
bt_tab5_fileOpen.grid(row=4, column=0, padx=1, pady=2, sticky=W+E+N+S)

bt_tab5_folderOpen = tk.Button(tab5, text='폴더 열기', relief="groove", overrelief="solid", command=fn_tab5_folderOpen, width=30)
bt_tab5_folderOpen.grid(row=4, column=1, padx=1, pady=2, sticky=W+E+N+S)

bt_tab5_clear = tk.Button(tab5, text='화면 초기화', relief="groove", overrelief="solid", command=fn_tab5_clear)
...
bt_tab6_watermarkfilename = tk.Button(tab6, text='워터마크 PDF 파일 열기', relief="groove", overrelief="solid", command=fn_tab6_setWaterMarkFile)
...
bt_tab6_setPDFFile = tk.Button(tab6, text='PDF 파일 열기', relief="groove", overrelief="solid", command=fn_tab6_setPDFFile) #fn_tab6_setPDFFile)
...
bt_tab6_Save = tk.Button(tab6, text='저장', relief="groove", overrelief="solid", command=fn_tab6_Save)
...
bt_tab6_fileOpen = tk.Button(tab6, text='파일 열기', relief="groove", overrelief="solid", command=fn_tab6_fileOpen, width=30)
bt_tab6_fileOpen.grid(row=5, column=0, padx=1, pady=2, sticky=W+E+N+S)

bt_tab6_folderOpen = tk.Button(tab6, text='폴더 열기', relief="groove", overrelief="solid", command=fn_tab6_folderOpen, width=30)
bt_tab6_folderOpen.grid(row=5, column=1, padx=1, pady=2, sticky=W+E+N+S)

bt_tab6_clear = tk.Button(tab6, text='화면 초기화', relief="groove", overrelief="solid", command=fn_tab6_clear)
...
def apply_skin(widget):
   ...
   def apply_widget_skin(w):
      new_font = font.Font(family="Spoqa Han Sans Neo Regular", size=11) #, weight="bold")
      ...
      if isinstance(widget, tk.Menu):
         ...
         pass
      elif isinstance(w, (tk.Button, tk.Entry)):
         w.configure(font=new_font)         
